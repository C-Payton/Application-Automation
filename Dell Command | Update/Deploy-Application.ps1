...
Try {
	...
	[string]$appVersion = '4.5.0'
	[string]$appArch = ''
	[string]$appLang = 'EN'
	[string]$appRevision = '01'
	[string]$appScriptVersion = '1.14.0'
	[string]$appScriptDate = '4/14/2022'
	...
	[string]$appDefaultTypeCategory = 'bios,firmware,driver,application,others,utility'
	[string]$appLicense = ""
	[array]$appIcons = @( )
	...
	If ($deploymentType -ine 'Uninstall' -and $deploymentType -ine 'Repair') {
		...
		if ($psArchitecture -eq "x86") {
			Show-InstallationPrompt -Message "This APP Package Does Not Include 32bit OS Support... Please Contact Support About a Computer Upgrade...." -ButtonRightText 'EXIT' -Icon Information -NoWait
			Exit $mainExitCode
		}
		...
		# Check and Uninstall UWP version before continuing 
		If (Get-InstalledApplication -Name "$appRegName for Windows 10"){
			Show-InstallationProgress "Detected a Previous $appRegName UWP Install... Removing old Version..."
			
			Remove-MSIApplications "$appRegName for Windows 10"
		}
		...
		If (!(Test-Path "$envProgramFilesX86\$appVendor\CommandUpdate\dcu-cli.exe")){
		...
		start-process -FilePath "$envProgramFilesX86\$appVendor\CommandUpdate\dcu-cli.exe" -ArgumentList "/configure -silent -biosPassword=`"SecretPassword`" -autoSuspendBitLocker=enable -userConsent=disable" -Wait -WindowStyle Hidden
		start-process -FilePath "$envProgramFilesX86\$appVendor\CommandUpdate\dcu-cli.exe" -ArgumentList "/configure -silent -scheduleManual -advancedDriverRestore=disable" -Wait -WindowStyle Hidden
		
		#Reset to the Default Update Type Settings
		start-process -FilePath "$envProgramFilesX86\$appVendor\CommandUpdate\dcu-cli.exe" -ArgumentList "/configure -silent -updateType=`"$appDefaultTypeCategory`"" -Wait -WindowStyle Hidden
		
		#Reset to the Default Update Device Settings
		start-process -FilePath "$envProgramFilesX86\$appVendor\CommandUpdate\dcu-cli.exe" -ArgumentList "/configure -silent -updateDeviceCategory=`"$appDefaultDeviceCategory`"" -Wait -WindowStyle Hidden
		...
		#start-process -FilePath "$envProgramFilesX86\$appVendor\CommandUpdate\dcu-cli.exe" -ArgumentList "/scan -silent -report=`"$envSystemDrive\temp\$appVendor Update`"" -Wait -WindowStyle Hidden
		...
		switch ($UpdateType)
		{
			"All"
			{
				...
				start-process -FilePath "$envProgramFilesX86\$appVendor\CommandUpdate\dcu-cli.exe" -ArgumentList "/applyUpdates -silent -outputLog=`"$envSystemDrive\temp\$appVendor Update\dell_update.log`" -reboot=disable" -Wait -WindowStyle Hidden -ErrorAction SilentlyContinue
			}
			"Drivers"
			{
				...
				start-process -FilePath "$envProgramFilesX86\$appVendor\CommandUpdate\dcu-cli.exe" -ArgumentList "/configure -silent -updateType=driver,application,others,utility" -Wait -WindowStyle Hidden
			...
			"Bios"
			{
				...
				start-process -FilePath "$envProgramFilesX86\$appVendor\CommandUpdate\dcu-cli.exe" -ArgumentList "/configure -silent -updateType=bios,firmware" -Wait -WindowStyle Hidden
			...
			"Network"
			{
				...
				start-process -FilePath "$envProgramFilesX86\$appVendor\CommandUpdate\dcu-cli.exe" -ArgumentList "/configure -silent -updateType=driver" -Wait -WindowStyle Hidden
				start-process -FilePath "$envProgramFilesX86\$appVendor\CommandUpdate\dcu-cli.exe" -ArgumentList "/configure -silent -updateDeviceCategory=network" -Wait -WindowStyle Hidden
			...
			"Display"
			{
				...
				start-process -FilePath "$envProgramFilesX86\$appVendor\CommandUpdate\dcu-cli.exe" -ArgumentList "/configure -silent -updateType=driver" -Wait -WindowStyle Hidden
				start-process -FilePath "$envProgramFilesX86\$appVendor\CommandUpdate\dcu-cli.exe" -ArgumentList "/configure -silent -updateDeviceCategory=video" -Wait -WindowStyle Hidden
		...
		If (get-content -path "$envSystemDrive\temp\$appVendor Update\dell_update.log" | select-string -pattern "$dateCheck \w{2}:\w{2}:\w{2}] : The program exited with return code: 500"){
			If (-not $useDefaultMsi) { Show-InstallationPrompt -Message "$installTitle Detected System is Already Fully Patched... If you have any Questions Please Contact CENG IT Support." -ButtonRightText 'OK' -Icon Information -NoWait }
		}
		...
		Else { 
			...
			If (-not $useDefaultMsi) { Show-InstallationPrompt -Message "$installTitle Has Updated System Drivers... If you have any Questions Please Contact CENG IT Support." -ButtonRightText 'OK' -Icon Information -NoWait }
		}
	}
